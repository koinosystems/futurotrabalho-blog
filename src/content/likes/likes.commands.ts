...
likesCommands.post("/toggle", authenticate, async (c) => {
  ...
  const existing = await c.env.BLOG_DB.prepare(
    "SELECT id FROM ft_content_likes WHERE content_id = ? AND user_id = ?",
  )
  ...
  if (existing) {
    await c.env.BLOG_DB.prepare(
      "DELETE FROM ft_content_likes WHERE content_id = ? AND user_id = ?",
    )
      .bind(parsed.data.contentId, userId)
      .run();
  ...
  await c.env.BLOG_DB.prepare(
    "INSERT INTO ft_content_likes (id, content_id, user_id, created_at) VALUES (?, ?, ?, datetime('now'))",
  )
