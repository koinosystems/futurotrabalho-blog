...
contentCommands.post("/", authenticate, async (c) => {
  ...
  const existing = await c.env.BLOG_DB.prepare(
    "SELECT id FROM ft_content WHERE slug = ?",
  )
    .bind(slug)
    .first();
  if (existing) {
  ...
  await c.env.BLOG_DB.prepare(
    `INSERT INTO ft_content (id, author_id, title, content, slug, summary, type, cover_img, cover_alt, status, created_at, updated_at)
  ...
  if (parsed.data.tags && parsed.data.tags.length > 0) {
    const tagStatements = parsed.data.tags.map((tag) =>
      c.env.BLOG_DB.prepare(
        "INSERT INTO ft_content_tags (id, content_id, tag) VALUES (?, ?, ?)",
...
contentCommands.put("/:id/publish", authenticate, async (c) => {
  ...
  const content = await c.env.BLOG_DB.prepare(
    "SELECT author_id FROM ft_content WHERE id = ?",
  )
    .bind(contentId)
  ...
  await c.env.BLOG_DB.prepare(
    "UPDATE ft_content SET status = 'published', published_at = datetime('now'), updated_at = datetime('now') WHERE id = ?",
  )
    .bind(contentId)
    .run();
...
contentCommands.patch("/:id", authenticate, async (c) => {
  ...
  if (updates.length > 0) {
    ...
    await c.env.BLOG_DB.prepare(
      `UPDATE ft_content SET ${updates.join(", ")} WHERE id = ?`,
    )
      .bind(...values)
      .run();
  }
  ...
  if (parsed.data.tags) {
    await c.env.BLOG_DB.prepare(
      "DELETE FROM ft_content_tags WHERE content_id = ?",
    )
      .bind(contentId)
      .run();
...
contentCommands.delete("/:id", authenticate, async (c) => {
  ...
  await c.env.BLOG_DB.prepare("DELETE FROM ft_content WHERE id = ?")
    .bind(contentId)
    .run();
