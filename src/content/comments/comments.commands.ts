...
commentsCommands.post("/", authenticate, async (c) => {
  ...
  await c.env.BLOG_DB.prepare(
    `INSERT INTO ft_content_comments (id, content_id, user_id, body, parent_id, status, created_at, updated_at)
...
commentsCommands.delete("/:id", authenticate, async (c) => {
  ...
  const comment = await c.env.BLOG_DB.prepare(
    "SELECT user_id FROM ft_content_comments WHERE id = ?",
  )
    .bind(commentId)
  ...
  await c.env.BLOG_DB.prepare("DELETE FROM ft_content_comments WHERE id = ?")
    .bind(commentId)
    .run();
