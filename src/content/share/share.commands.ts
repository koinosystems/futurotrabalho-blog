...
shareCommands.post("/register", async (c) => {
  ...
  const existing = await c.env.BLOG_DB.prepare(
    "SELECT id, count FROM ft_content_share_counts WHERE content_id = ? AND platform = ?",
  )
  ...
  if (existing) {
    await c.env.BLOG_DB.prepare(
      "UPDATE ft_content_share_counts SET count = count + 1, updated_at = datetime('now') WHERE id = ?",
    )
      .bind(existing.id)
      .run();
  } else {
    await c.env.BLOG_DB.prepare(
      "INSERT INTO ft_content_share_counts (id, content_id, platform, count, created_at, updated_at) VALUES (?, ?, ?, 1, datetime('now'), datetime('now'))",
    )
